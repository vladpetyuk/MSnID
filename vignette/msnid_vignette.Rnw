...
% \VignetteDepends{BiocStyle, msmsTests}
...
ggplot(x, aes(x=dM, fill=isDecoy)) + geom_histogram(position='stack')
@
\begin{center}
...
system.time({
   filtObj.sann <- optimize_filter(filtObj.grid, msnid, fdr.max=0.01, 
                                  method="SANN", level="Peptide", n.iter=500)})
show(filtObj.sann)
# (absParentMassErrorPPM < 2.2) & (msmsScore > 7.6)
@
Let's check which filter turned out to be the best. The differece between the optimized filters is likely to be very small.
<<>>=
evaluate_filter(msnid, filtObj, level="Peptide")
...
@
We'll choose and apply filter produced by simulated annealing optimization procedure. Identifications that matched to decoy and contaminant protein sequences can be removed by providing filters in the forms of text strings that will be evaluated in the context of PSM table.
<<>>=
...
validObject(msnset)
@
Currently the features in the \Rclass{MSnSet} object are peptides. However, we plan to quantify differences at level of relative protein abundances. Peptides can be rolled-up to protein level using \Rfunction{combineFeatures} function from \Biocpkg{MSnbase}. Ambiguity in peptide-to-protein assignment is handled by \Rcode{redundancy.handler} argument. It can allow ambiguity of peptide assignment to \emph{multiple} proteins or \emph{unique} mapping only by discarding peptide mapping two or more proteins.
<<label=assessingRedundancy, fig=TRUE, include=FALSE>>=
...
@
\begin{center}
\includegraphics[width=0.8\textwidth]{graphics-assessingRedundancy}
\end{center}
\pagebreak[4]
<<label=rollingToProteinLevel>>==
...
length(featureNames(msnset))
@
For statistical testing we will leverage functionality available in \Biocpkg{msmsTests}. Here we are comparing the difference between \textit{glp-4; daf-2} long-living strain with a strain that bears compensatory mutation within \textit{daf-16} transcription factor. For statistical testing of the count data we will use quasi-Poisson model \Rfunction{msms.glm.qlll}.
<<label=statisticalTests, fig=TRUE, include=FALSE>>=
library("msmsTests")
...
@
\begin{center}
\includegraphics[width=0.4\textwidth]{graphics-statisticalTests}
\end{center}
\pagebreak[0]
