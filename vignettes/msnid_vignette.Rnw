...
\section{Setting up working directory}
...
show(msnid)
@
Alternative and currently the preferred way is to read MS/MS results is by 
parsing mzIdentML files (*.mzid or *.mzid.gz extensions) using \Biocpkg{mzID} 
package facilities.
<<>>=
...
\section{Exploring the \Robject{MSnID} object}
...
\section{Parent ion mass measurement accuracy}
...
\cite{Piehowski2013a}. This should be good starting parameters for 
follow-up refining optimizations.
<<>>=
system.time({
...
The resulting \Rcode{filtObj.grid} can be further refined with such 
...
Bioconductor \Rclass{eSet} to quantitative proteomics data. The spectral count
data can be analyzed with \Biocpkg{msmsEDA}, \Biocpkg{msmsTests} or 
\Biocpkg{DESeq}.
