...
criteria and ensuring confidence in MS/MS identification data. User can 
...
<<>>=
yourPSMresults <- read.delim(system.file("extdata", "human_brain.txt", 
                                            package="MSnID"),
...
parsing mzIdentML files (*.mzid or *.mzid.gz extensions). The read\_mzIDs 
function levelrages \Biocpkg{mzID} package facilities.
<<>>=
...
sequence creates new column numIrregCleavages.
<<>>=
msnid <- assess_termini(msnid, validCleavagePattern="[KR]\\.[^P]")
@
Counting the number of missed cleavages in peptides sequence creates new 
column numMissCleavages.
<<>>=
...
settings of the MS/MS search engine MS-GF+ (used for analysis of this 
LC-MS dataset) fairly assumed that the instrument could have peak 
non-monoisotopic peaks of parent ion for fragmentation 
and thus considered peptides that were off by +/- 1 Da
(\textsuperscript{13}C-\textsuperscript{12}C to be exact).
...
Decon2LS \cite{Mayampurath2008} tools that identifies the monoisotipic peaks 
post-experimentally. \Biocpkg{MSnID} package provide a simple 
\Rcode{correct_peak_selection} function that simply adds or subtracts 
...
For further processing we'll consider \Rcode{msnid.chopped} data that ignores
matches with 1 Da errors. Note, if the center of the histogram is 
significantly shifted from zero, \Rcode{experimentalMassToCharge} can 
be post-experimentally recalibrated. This MS/MS data was preprocessed with 
...
Let's keep it under the "msmsScore" name. The score density distribution
...
\end{center}
\pagebreak[0]
As a second criteria we will be using absolute mass measurement 
error (in ppm units) of the parent ion. The mass measurement error tend to
...
\section{Optimizing MS/MS filter to achieve the maximum number of 
identifications within given FDR upper limit threshold}
...
@
Finally we'll apply optimized filter to proceed with further 
...
\section{Downstream quantitative analysis}
...
to \Rclass{MSnSet} object defined in \Biocpkg{MSnbase} that extends generic 
